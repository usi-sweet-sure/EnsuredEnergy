...
namespace Godot {
	...
	public struct Capacity : IColumn {
		...
		public Capacity(
			float gas=0, float nuclear=0, float river=0,
			float hydro=0, float pump=0, float waste=0,
			float biomass=0, float solar=0, float wind=0
		) {
			Gas = gas;
			Nuclear = nuclear;
			River = river;
		...
		public float _GetField(Building b) => b.type switch {
			Building.Type.GAS     => Gas,
			Building.Type.NUCLEAR => Nuclear,
			Building.Type.RIVER   => River,
			Building.Type.HYDRO   => Hydro,
			Building.Type.PUMP    => Pump,
			Building.Type.WASTE   => Waste,
			Building.Type.BIOMASS => Biomass,
			Building.Type.SOLAR   => Solar,
			Building.Type.WIND    => Wind,
		...
		// Sum all the the values stored here
		public readonly float Aggregate() => 
			Gas + Nuclear + River +
			Hydro + Pump + Waste +
			Biomass + Solar + Wind;
		...
		public readonly bool IsEmpty() => 
			Gas == Nuclear && Nuclear == River && River == Hydro &&
			Hydro == Pump && Pump == Waste && Waste == Biomass &&
			Solar == Wind && Wind == 0.0f;
	}
